...
def test_all_close_rerope(Z, H, N_CTX, D_HEAD, WINDOW, causal, dtype=torch.float16, device='cuda'):
    ## generate qkv data
    ...
    def check_diff(out1, out2):
        ...
        return len(cnt)
    assert torch.allclose(torch_out, tri_out, atol=1e-2, rtol=2), f"The output is different with {check_diff(torch_out, tri_out)} tokens"

# test_all_close_rerope(1, 2, 2048, 64, 512, True)
# print("They are all close!\n")
...
bench_flash_attention_with_rerope.run(save_path="./data/flash_attn_triton_results", print_data=True)